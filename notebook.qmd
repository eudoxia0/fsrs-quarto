...
def sim(grades: list[Grade]) -> list[tuple[T, Grade, S, D]]:
    t: T = 0.0
    l: list[tuple[T, Grade, S, D]] = []
...
def sim2df(r: list[tuple[T, Grade, S, D]]):
    return pd.DataFrame(r, columns=['t', 'g', 's', 'd'])
```

All easy:

```{python}
sim2df(sim([G3, G3, G3, G3]))
```

All good:

```{python}
sim2df(sim([G2, G2, G2, G2]))
```

All hard:

```{python}
sim2df(sim([G1, G1, G1, G1]))
```

All forgot:

```{python}
sim2df(sim([G0, G0, G0, G0]))
```
